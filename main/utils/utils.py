...
from swift.llm.dataset.preprocessor import DATASET_TYPE, RowPreprocessor
...
class LazyLLMDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        for i in range(self.n_try_fetch):
            ...
            data = self.dataset[i]
            try:
                mos = re.search(r'\((.*?)\)', data["messages"][1]["content"]).group(1)
                data["messages"][1]["content"] = re.sub(r'\s*\(.*?\)', '', data["messages"][1]["content"])
                ret = self.encode_func(data)
                ret["mos"] = torch.tensor(float(mos)/100, dtype=torch.bfloat16, requires_grad=True)
