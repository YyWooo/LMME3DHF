...
from swift.llm.utils import Processor, ProcessorMixin
from swift.llm.template.template_inputs import InferRequest, StdTemplateInputs, TemplateInputs
from swift.llm.template.utils import Context, ContextType, StopWordsCriteria, fetch_one, findall, split_str_parts_by
from swift.llm.template.vision_utils import load_image, rescale_image
...
class Template(ProcessorMixin):
    ...
    def _data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        ...
        res["video_grid_thw"] = torch.tensor([b['video_grid_thw'][0].tolist() for b in batch if b.get('video_grid_thw') is not None])
        res["mos"] = [b['mos'] for b in batch if b.get('mos') is not None]
        return res
