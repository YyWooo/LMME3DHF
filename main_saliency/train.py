...
def train_epoch(epoch, nEpochs, data_loader, model, decoder, tokenizer, optimizer, criterion, opt,
    ...
    if not opt.mm_train:
        for name, param in model.named_parameters():
            if "readout" not in name: # and "model.layers.0.mlp.gate_proj" not in name:
                assert param.requires_grad == False
            else:
                assert param.requires_grad == True
    else:
        for name, param in model.named_parameters():
            if "mm_projector" not in name: # and "model.layers.0.mlp.gate_proj" not in name:
                assert param.requires_grad == False
            else:
                assert param.requires_grad == True
    ...
    for i, (data, target) in enumerate(data_loader):
        ...
        loss = loss / opt.update_step
        loss = (loss - opt.b).abs() + opt.b
        loss.backward()
        ...
        if (i+1) % opt.update_step == 0:
            ...
            batch_logger.log({
                ...
                'loss': "{:.4f}".format(loss),
                # 'rec_loss': rec_loss,
                'nll_loss': "{:.4f}".format(nll_loss),
                'CC_loss': "{:.4f}".format(CC_loss),
    ...
    epoch_logger.log({
        'epoch': epoch,
        'epoch_time': "{:.2f}".format(time.time() - epoch_begin_time),
