...
def save_salmap(data, filename):
    data = data.squeeze().cpu().detach()  # 去掉 batch 和 channel 维度，并转移到 CPU
    data = data.numpy()
    ...
    image = Image.fromarray(data_norm, mode='L')  # 'L' 表示灰度图
    resized_image = image.resize((1536, 512), Image.BILINEAR)  # 使用双线性插值
    resized_image.save(filename)  # 保存为文件
    
def salmap_loader(path, version, backbone):
	# open path as file to avoid ResourceWarning (https://github.com/python-pillow/Pillow/issues/835)
    # img = cv2.imread(path, 0) # Load as grayscale # [480,960] [h,w]
    img = Image.open(path).convert("L")
    img = np.array(img)

    if backbone == "VideoLLaMA2.1-7B-16F":
        if version in  ["ver2", "ver14"]:
            img = cv2.resize(img, (208,208), interpolation=cv2.INTER_AREA) 
        elif version in ["ver3" ,"ver5", "ver18", "ver19", "ver20", "ver21"]:
            img = cv2.resize(img, (216,216), interpolation=cv2.INTER_AREA) 
        elif version in ["ver4", "ver6", "ver7", "ver8", "ver9", "ver10", "ver11", "ver12", "ver13", "ver15", "ver16", "ver17"]:
            img = cv2.resize(img, (223,223), interpolation=cv2.INTER_AREA) 
    elif backbone == "VideoLLaMA2-7B":
        if version in  ["ver2", "ver14"]:
            img = cv2.resize(img, (208,208), interpolation=cv2.INTER_AREA) 
        elif version in ["ver3" ,"ver5", "ver18", "ver19", "ver20", "ver21"]:
            img = cv2.resize(img, (192,192), interpolation=cv2.INTER_AREA) 
        elif version in ["ver4", "ver6", "ver7", "ver8", "ver9", "ver10", "ver11", "ver12", "ver13", "ver15", "ver16", "ver17"]:
            img = cv2.resize(img, (199,199), interpolation=cv2.INTER_AREA) 
    if np.max(img) != 0:
        img = (img-np.min(img))/(np.max(img)-np.min(img))
    else:
        print(path)
        with open('image_log.txt', 'a') as log_file:
...
def process_image(image_path, processor, aspect_ratio='pad'):
    image = Image.open(image_path).convert('RGB') # sora.png:1920x1080 [w,h]

    images = [np.array(image)] # [h w c]
    ...
    if aspect_ratio == 'pad':
        ...
        images = [Image.fromarray(f) for f in images] # 变回来了 [w,h]

    images = processor.preprocess(images, return_tensors='pt')['pixel_values']
    return images # torch.Size([1, 3, 384, 384])


def frame_sample(duration, mode='uniform', num_frames=None, fps=None): # 改
    if mode == 'uniform':
...
def process_video(video_path, processor, s=None, e=None, aspect_ratio='pad', num_frames=NUM_FRAMES):
    ...
    if aspect_ratio == 'pad':
        ...
        video = processor.preprocess(images, return_tensors='pt')['pixel_values']
    return video # torch.Size([16, 3, 384, 384]) # 2.0 [8,3,336,336]


def process_myvideo(sampled_frames, processor, aspect_ratio='pad', num_frames=NUM_FRAMES):
    images = [Image.open(f_idx) for f_idx in sampled_frames] # [w,h]
    video = processor.preprocess(images, return_tensors='pt')['pixel_values']
    return video # torch.Size([16, 3, 384, 384])
...
def tokenizer_multimodal_token(prompt, tokenizer, multimodal_token=DEFAULT_IMAGE_TOKEN, return_tensors=None):
    """Tokenize text and multimodal tag to input_ids. input_ids 是分词器将每个 token 映射成整数 ID 的结果
...
class KeywordsStoppingCriteria(StoppingCriteria):
    ...
    def call_for_batch(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        offset = min(output_ids.shape[1] - self.start_len, self.max_keyword_len) # offset 计算的作用是只检查生成的最后一部分内容（相对于初始输入而言），长度不超过最长关键词长度。这样可以减少不必要的计算。
    ...
    def __call__(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        outputs = [] # 生成完新的首先进入这里判断 ③⑦
        for i in range(output_ids.shape[0]): # torch.Size([1, 1])->torch.Size([1, 2])
            outputs.append(self.call_for_batch(output_ids[i].unsqueeze(0), scores))
        return all(outputs) # all(outputs) 会检查 outputs 中是否所有样本都满足停止条件。如果 outputs 中所有值均为 True，则 all(outputs) 返回 True，表示整个批次的生成过程应停止；否则继续生成。
