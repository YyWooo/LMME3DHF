...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    elif not 'torch_dtype' in kwargs:
        kwargs['torch_dtype'] = torch.float16

    if use_flash_attn: # False
        kwargs['attn_implementation'] = 'flash_attention_2'

    config = AutoConfig.from_pretrained(model_path) # 如果路径中有一个 config.json 文件，AutoConfig 会读取其中的内容（JSON 格式）.在 AutoConfig 读取到 model_type 为 "videollama2_qwen2" 后，它会在 VLLMConfigs 这个映射表中查找对应的类，一旦找到了与 model_type 对应的配置类，就会实例化该类，因此最终调用config = Videollama2Qwen2Config.from_pretrained(model_path)
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False, token=token) # 在 Hugging Face Hub 或本地路径下，AutoTokenizer 会查找并读取这些文件：tokenizer_config.json：指定分词器的类型和相关设置。vocab.txt 或 merges.txt：用于词汇表或 BPE（Byte Pair Encoding）合并规则。special_tokens_map.json：存储模型的特殊标记（如 [CLS] 和 [PAD]）。
        ...
        if model_type in ['videollama2', 'videollama2_mistral']:
            ...
            model = Videollama2Qwen2ForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,cache_dir="./.cache/", config=config, **kwargs)
        else:
            model = Videollama2MistralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, config=config, **kwargs)
    else:
