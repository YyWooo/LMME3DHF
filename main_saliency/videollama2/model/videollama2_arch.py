...
class Videollama2MetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if self.get_vision_tower() is None:
            vision_tower = build_vision_tower(model_args) # torch.float32
        ...
        if getattr(self, 'mm_projector', None) is None:
            self.mm_projector = build_vision_projector(self.config) # 刚加载进来是float32的
        else:
...
class Videollama2MetaForCausalLM(ABC):
    ...
    def encode_images_or_videos(self, images, vision_select_layer=None):
        num_frames = self.config.num_frames if hasattr(self.config, 'num_frames') else NUM_FRAMES

        data_batch = []
        for i, (data, modal) in enumerate(images): # bs概念
            if modal == 'image':
        ...
        data_batch = torch.stack(data_batch, dim=0) # image:torch.Size([1, 16, 3, 384, 384]) # 2.0 torch.Size([1, 8, 3, 336, 336])

        assert len(data_batch.size()) == 5
        batch_size = data_batch.size(0)

        frames = einops.rearrange(data_batch, 'b t c h w -> (b t) c h w') # [16, 3, 384, 384] # 2.0 [8,3,336,336]
        visual_embedding, hidden_states = self.get_model().get_vision_tower()(frames,vision_select_layer) # [16, 729, 1152]，其中729=27*27 # 2.0 [1,8,576,1024], 576=24*24
        frames_features = einops.rearrange(visual_embedding, '(b t) n h -> b t n h', b = batch_size) # ->[1, 16, 729, 1152] # 2.0 [1,8,576,1024]

        return self.temporal_aggregator(frames_features), hidden_states#features_12, features_22, features_15, features_27 # image:[1, 16, 729, 1152]->[1, 1352, 3584]，其中1352=8*13*13
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, vision_select_layer=None
    ):
        ...
        mm_features, hidden_states = self.encode_images_or_videos(images,vision_select_layer) # image:torch.Size([1, 1352, 3584])，其中1352=8*13*13 # 2.0 torch.Size([1, 845, 4096]) 其中845=5*13*13
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            num_multimodals = sum((cur_input_ids == mm_token_idx).sum() for mm_token_idx in MODAL_INDEX_MAP.values()) # 1
            # pure text input
            ...
            while mm_token_indices.numel() > 0:
                ...
                vtoken_start = mm_token_start
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[:mm_token_start])) 
                cur_new_input_embeds.append(cur_mm_features) # 列表第一个是第一部分文本特征（系统提示），第二个是图像特征
                vtoken_end = mm_token_start + cur_mm_features.shape[0] - 1
                if labels is not None:
            ...
            if cur_input_ids.numel() > 0:
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids)) # 列表第三个是第二部分文本特征（用户instruct）
                if labels is not None:
                    cur_new_labels.append(cur_labels)
            cur_new_input_embeds = [x.to(device=self.device) for x in cur_new_input_embeds]
            # NOTE: one cur_new_input_embeds per each  
            cur_new_input_embeds = torch.cat(cur_new_input_embeds, dim=0) # [14,3584] [1352,3584] [24,3584] -> [1390,3584]
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            new_input_embeds = torch.stack(new_input_embeds, dim=0) # [1, 1390, 3584] # 2.0 torch.Size([1, 998, 4096])
            if labels is not None:
                new_labels  = torch.stack(new_labels, dim=0)

            if attention_mask is not None:
                new_attn_mask_pad_left = torch.full((attention_mask.shape[0], new_input_embeds.shape[1] - input_ids.shape[1]), True, dtype=attention_mask.dtype, device=attention_mask.device)
                attention_mask = torch.cat((new_attn_mask_pad_left, attention_mask), dim=1) # [1, 1351]的全1和torch.Size([1, 39]的全1拼接->[1, 1390]的全1
                assert attention_mask.shape == new_input_embeds.shape[:2]

        return None, attention_mask, past_key_values, new_input_embeds, new_labels, vtoken_start, vtoken_end, hidden_states
