...
class STCConnector(nn.Module):
    ...
    def __init__(self, config, downsample=(2, 2, 2), depth=4, mlp_depth=2):
        ...
        """
        super().__init__()
        self.encoder_hidden_size = encoder_hidden_size = config.mm_hidden_size  # 输入的特征维度（来自视觉模型的特征）
        self.hidden_size = hidden_size = config.hidden_size  # 中间层大小
        self.output_hidden_size = output_hidden_size = config.hidden_size  # 输出隐藏层大小
    ...
    def forward(self, x): # image:torch.Size([1, 16, 729, 1152])
        ...
        """
        # x = x.to(self.s1.b1.conv1.conv.weight.dtype)
        t = x.size(1) # 16
        if x.ndim == 4:
            hw = int(x.size(2) ** 0.5)
            x = einops.rearrange(x, "b t (h w) d -> b d t h w", h=hw, w=hw) # torch.Size([1, 1152, 16, 27, 27]) # 2.0 [1, 1024, 8, 24, 24]
        elif x.ndim == 5:
            x = einops.rearrange(x, "b t h w d -> b d t h w")

        x = einops.rearrange(x, "b d t h w -> (b t) d h w") # -> torch.Size([16, 1152, 27, 27]) # 2.0 [8,1024,24,24]
        # 1. the first stage of the adapter
        x = self.s1(x) # ->经过一堆conv等操作[16, 3584, 27, 27] # 2.0 [8,4096,24,24]
        x = einops.rearrange(x, "(b t) d h w -> b d t h w", t=t) # ->[1, 3584, 16, 27, 27] # 2.0 [1,4096,8,24,24]
        # 2. downsampler
        x = self.sampler(x) # ->[1,3584,8,13,13] Conv3d(3584, 3584, kernel_size=(2, 2, 2), stride=(2, 2, 2))
        new_t = x.size(2)
        # 3. the second stage of the adapter
        x = einops.rearrange(x, "b d t h w -> (b t) d h w") # [8, 3584, 13, 13] # 2.0 [5,4096,13,13]
        x = self.s2(x) # [8, 3584, 13, 13]
        x = einops.rearrange(x, "(b t) d h w -> b (t h w) d", t=new_t) # torch.Size([1, 1352, 3584]) 其中1352=8*13*13 # 2.0 torch.Size([1, 845, 4096])，其中845=5*13*13
        x = self.readout(x)
        return x
