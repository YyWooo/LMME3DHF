...
def train(attn_implementation=None):
    ...
    if training_args.bits in [4, 8]: # bits=16, False
    ...
    if model_args.vision_tower is not None:
        model = VLLMs[model_args.model_type].from_pretrained(
            ...
            config=config,
            torch_dtype=(torch.bfloat16 if training_args.bf16 else None), # 以torch.bfloat16格式加载Qwen
            do_sample=True,
            **bnb_model_from_pretrained_args
        )
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(model), # ['q_proj', 'up_proj', 'down_proj', 'o_proj', 'gate_proj', 'k_proj', 'v_proj']
    ...
    with open('model_train_483.txt', "w") as f: # lora部分是float32，其他都是bfloat16
        f.write("Model Parameters Data Types:\n\n")
        for name, param in model.named_parameters():
            f.write(f"Parameter Name: {name}, Data Type: {param.dtype}\n")
        f.write("\nBuffers Data Types:\n\n")
        for name, buffer in model.named_buffers():
            f.write(f"Buffer Name: {name}, Data Type: {buffer.dtype}\n")
    ...
    if model_args.vision_tower is not None:
        ...
        vision_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device) # 将视觉编码器推到torch.bfloat16
        ...
        if training_args.freeze_mm_mlp_adapter: # False
            for p in model.get_model().mm_projector.parameters():
